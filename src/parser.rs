...
// Parse hints generated by AP-Playthrough-hinter
pub fn stored_hint_parser<'src>() -> impl Parser<'src, &'src str, Vec<SpoilerEntry>> {
    let entry = empty()
        ...
        .then_ignore(just("\n").or_not())
        .map(|(((loc, item), sender), receiver)| SpoilerEntry {
            location: loc,
            item,
            sender,
            receiver,
